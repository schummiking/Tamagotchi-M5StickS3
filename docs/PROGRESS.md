...
| 2026-05-02 | 两键组合 C 采用释放判定的实体键语义 | 短按松开才发送 A/B，长按松开才触发设置；按住一个键期间出现另一个键立即判定为组合键，避免时钟设置误加小时 |
| 2026-05-02 | `key1 -> key2` 保持原版 C/退出，不复用为睡眠入口 | 用户实机操作里 C/退出是高频核心操作；低功耗应走自动显示策略，不能抢组合键语义 |
| 2026-05-03 | `key2 -> key1` 映射原版 A+C/SET | P1 时钟页需要 A+C 进入 SET；按下顺序区分后，设备端可直接改时钟，不必依赖 USB 调试口 |
...
| 2026-05-03 | 开发 | 顺序组合键修正：`key1 -> key2` 继续发送原版 C/退出，`key2 -> key1` 发送原版 `A+C`/SET，释放阶段不再补发 A/B | 本次提交 |
...
- `src/buttons.*`：`key1`/`key2`/顺序组合键到 A/B/C/A+C 的实时 mask
