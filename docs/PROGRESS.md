...
项目已完成阶段 3 到可交付状态：带本地 P1 ROM 的固件可启动、可恢复 LittleFS 存档、可从 NVS 恢复亮度/音量配置，空闲 30 秒后会降低屏幕亮度。当前状态栏修正版串口确认 `storage: restored 648 bytes` 和 `boot ok: M5StickS3 phase3-ui-status-001`。
...
- 完成阶段 3：LittleFS 存档/恢复、NVS 亮度/音量配置、状态栏亮度/音量档位、idle 降亮、黑房间渲染、竖屏菜单图标、显示睡眠和低电压/睡眠前强制保存入口
...
| 任务 | 状态栏修正：显示亮度/音量档位 |
| 状态 | 已完成并验证 |
| 验收标准 | 移除会随蜂鸣瞬时变化的 `sound:on/off`；状态栏显示当前亮度和音量 3 格档位；文档说明长按 `key1` 调亮度、长按 `key2` 调音量；编译、烧录、串口启动验证通过 |
...
| 阶段 3：存档和功耗 | 已完成到可交付状态 | 重启可恢复 LittleFS 存档；亮度/音量 NVS 持久化；状态栏档位；空闲降亮；关灯黑房间；竖屏菜单图标；显示睡眠；低电压/睡眠前尽力保存 |
...
| 2026-05-02 | 文档 | 记录状态栏修正任务、亮度/音量操作方式和 git push 卡点：当前没有 remote | 本次提交 |
...
- `include/pins.h`：固件版本更新为 `phase3-ui-status-001`
...
- 长按 `key2` 后松开：循环音量档位 `32 -> 96 -> 160 -> 32`
