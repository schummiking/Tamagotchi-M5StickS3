...
- 手动确认 `key1`、`key2`、`power` 短按、`key1+key2`、`key2` 长按
- 确认启动短音和按钮短音是否可听
...
| 2026-05-02 | `power` 只使用短按，不使用长按/双击 | StickS3 标注和实测都表明该键长按进下载模式、双击关机 |
...
| 2026-05-02 | 发现 | 屏幕、按钮、声音都有反应，但长按 `power` 会进入下载模式 | 本次按键修正提交 |
| 2026-05-02 | 修正 | 根据 StickS3 标注图和 M5Unified API，改为 `key1`=A、`key2`=B、`power` 短按=C | 本次按键修正提交 |
| 2026-05-02 | 文档 | 统一按钮口径为 `power`、`key1`（蓝色正面键）、`key2`（侧键） | 本次按键修正提交 |
...
- 串口输出 `boot ok`：已完成
- 可以在没有 ROM 的情况下跑硬件 smoke test：已完成启动，待手动确认三键映射
