...
项目处于阶段 1：硬件验证/工程骨架，smoke test 已烧录并启动。
...
- 观察设备屏幕是否显示 `M5StickS3 smoke` / `boot ok`
- 手动按左键、右键、长按左键、双键，确认屏幕和串口事件
- 确认启动短音和按钮短音是否可听
...
| 2026-05-02 | 开发 | 开始阶段 1：M5StickS3 硬件 smoke test | 待提交 |
| 2026-05-02 | 开发 | 新增 PlatformIO/M5Unified smoke test 工程，覆盖屏幕、按钮、喇叭、IMU | 待提交 |
| 2026-05-02 | 验证 | `platformio run` 编译通过 | 待提交 |
| 2026-05-02 | 阻塞 | `COM3` 可识别但串口写入和 esptool 握手均超时，等待设备重新枚举/下载模式确认 | 待提交 |
| 2026-05-02 | 验证 | StickS3 下载模式枚举为 `COM4`，esptool 成功识别 ESP32-S3-PICO-1、8MB Flash、8MB PSRAM | 待提交 |
| 2026-05-02 | 验证 | smoke test 固件成功烧录到 `COM4` | 待提交 |
| 2026-05-02 | 验证 | 短按 reset 后从 flash 启动，串口输出 `boot ok: M5StickS3 phase1-smoke-001` 和 IMU 数据 | 待提交 |
...
- 串口输出 `boot ok`：已完成
- 可以在没有 ROM 的情况下跑硬件 smoke test：已完成启动，待手动确认屏幕/按钮/声音
