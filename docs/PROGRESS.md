...
项目已完成阶段 3 到可交付状态：带本地 P1 ROM 的固件可启动、可恢复 LittleFS 存档、可从 NVS 恢复亮度/音量配置，空闲 30 秒后会降低屏幕亮度。当前竖屏菜单 UI 版串口确认 `storage: restored 648 bytes` 和 `boot ok: M5StickS3 phase3-ui-menu-001`。
...
- 完成阶段 3：LittleFS 存档/恢复、NVS 亮度/音量配置、idle 降亮、黑房间渲染、竖屏菜单图标、显示睡眠和低电压/睡眠前强制保存入口
...
| 任务 | 竖屏 UI 修正：原版菜单图标区 |
| 状态 | 已完成并验证 |
| 验收标准 | 删除常驻按键说明；竖屏底部显示 8 个可读菜单图标和短标签；Attention 作为提示图标保留；编译、烧录、串口启动验证通过 |
...
| 阶段 3：存档和功耗 | 已完成到可交付状态 | 重启可恢复 LittleFS 存档；亮度/音量 NVS 持久化；空闲降亮；关灯黑房间；竖屏菜单图标；显示睡眠；低电压/睡眠前尽力保存 |
...
- `src/display.cpp`：竖屏运行界面显示原版 8 菜单图标和选中项提示
- `src/main.cpp`：接入设置快捷键和功耗更新
- `src/tama_app.cpp`：接入存档恢复、输入后 dirty 标记和 idle 保存
- `include/pins.h`：固件版本更新为 `phase3-ui-menu-001`
...
- 启动日志出现 `boot ok: M5StickS3 phase3-ui-menu-001`
