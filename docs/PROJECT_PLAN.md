...
- `src/power_manager.*` 实现 idle 降亮、暗屏夜间亮度、显示待机、夜间分段 deep sleep、唤醒后快速补偿 TamaLIB 时间、按键活动恢复亮度、低电压/睡眠前尽力保存
...
- 黑屏待机会保留绿色 LED，提示设备仍在运行；真实低功耗 sleep 才关闭 LED 并分段补偿时间
...
- 基础亮度/音量设置
- 无操作降亮度或关屏；宠物关灯后显示黑房间并可进入显示待机，待机时保留绿色 LED
- 关灯房间识别需要覆盖打呼动画：房间背景反色为黑，打呼/动画保留为绿色，并继续触发夜间降亮和显示睡眠
- 夜间黑房间持续一段时间后进入分段 deep sleep；进入真实低功耗前保存状态、关闭绿色 LED，定时唤醒后快速补跑睡眠时间并继续判断是否回到 sleep
