...
void settingsInit() {
  ...
  Serial.printf("settings: brightness=%u idle=%u volume=%u idle_dim=%lums\n",
                g_active_brightness, g_idle_brightness, g_volume,
...
uint8_t settingsCycleBrightness() {
  ...
  Serial.printf("settings: brightness=%u idle=%u\n", g_active_brightness,
                g_idle_brightness);
  return g_active_brightness;
}
