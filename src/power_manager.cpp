...
void writeSleepJournal(uint32_t sleep_ms) {
  Preferences prefs;
  ...
  prefs.end();
  Serial.printf("power: sleep journal set ms=%lu seq=%lu\n",
                static_cast<unsigned long>(sleep_ms),
                static_cast<unsigned long>(g_rtc_sleep_seq));
}

bool readSleepJournal(uint32_t& sleep_ms, uint32_t& sleep_seq) {
  Preferences prefs;
...
bool powerManagerUpdate(bool any_pressed, uint32_t idle_age_ms, bool game_screen_dark) {
  ...
  if (g_display_sleeping) {
    if (g_resume_low_power_soon && game_screen_dark) {
      g_resume_low_power_soon = false;
      enterLowPowerSleepChunk("night-chain");
    }
...
bool powerManagerRunLowPowerTest(uint32_t sleep_ms) {
  ...
  enterLowPowerSleepChunk("serial-test", static_cast<uint64_t>(sleep_ms) * 1000ULL);
  return false;
}
...
void powerManagerPrintDiagnostics() {
  ...
  Serial.printf("power: diag display_sleep=%d dimmed=%d pending=%d pending_ms=%lu pending_seq=%lu last_catchup_ms=%lu last_wake=%lu last_seq=%lu\n",
