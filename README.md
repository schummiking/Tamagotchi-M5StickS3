...
- 阶段 3：已完成 LittleFS 存档、NVS 配置、4 档音量含静音、亮度调节、空闲降亮、关灯黑房间和显示待机。自动 true deep sleep 已关闭，当前策略为时间准确优先；串口 `nap` 仍保留为手动诊断入口。
- 当前固件版本：`phase3-lowpower-006`。
...
- Phase 3: LittleFS save / restore, NVS settings, 4-level volume including mute, brightness control, idle dimming, dark-room rendering, and display standby are implemented. Automatic true deep sleep is disabled so time accuracy wins; serial `nap` remains available for manual diagnostics.
- Current firmware version: `phase3-lowpower-006`.
