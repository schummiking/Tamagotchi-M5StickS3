...
## 硬件
- M5StickS3 (ESP32-S3, 双核240MHz, 8MB Flash, 8MB PSRAM)
...
适配 S3 的工作：
- 屏幕驱动：ST7789 → LovyanGFX 或 TFT_eSPI，分辨率映射（原版 32x16 → 135x240 缩放）
- 按钮映射：原版 3 键（A/B/C）→ S3 的 2 键 + 长按/组合
...
## 里程碑
1. 到手后验证硬件：屏幕、按钮、喇叭、IMU 基本驱动
2. 移植 TamaLIB 到 S3，跑通原版 P1 ROM
3. 适配屏幕缩放、按钮映射、音频输出
4. 添加 IMU 互动和音效增强
