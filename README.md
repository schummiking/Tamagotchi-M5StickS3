...
- 阶段 3：已完成 LittleFS 存档、NVS 配置、4 档音量含静音、亮度调节、空闲降亮、关灯黑房间、显示待机、外部供电防 deep sleep、电池供电下的 idle/夜间分段 deep sleep 和唤醒后时间补偿。
- 当前固件版本：`phase3-lowpower-005`。
...
- Phase 3: LittleFS save / restore, NVS settings, 4-level volume including mute, brightness control, idle dimming, dark-room rendering, display standby, external-power deep-sleep guard, battery-only segmented idle/night deep sleep, and wake catch-up are implemented.
- Current firmware version: `phase3-lowpower-005`.
